...
- `index.html` — one page, six chapters: title page → the arc (metal, the leap, the startup; `data-sc-act="flow"`) → the AI era (`pin`, the inlined workspace SVG draws from `--sc-p`) → intertitle → proof (`flow`, iris reveal onto a phone plate that loads the Rolfe Legends 2 iframe on request, then the book club game and the second brain with a static vault diagram drawn in the same language as the workspace one) → beyond work (`scrub`, the one clip, in a column) → last page (`pin`, greet-and-hold cue, a slow-motion clubs loop that plays only while on screen, footer inside the stage). A hidden `#cv-print` block (direct child of `<body>`) is the ONLY thing visible in print.
